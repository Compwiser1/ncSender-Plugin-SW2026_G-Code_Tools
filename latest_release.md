...
**Material now cycles between aluminum, wood, and royal blue plastic** across separate uses of the plugin (not within one wait, since the animation is pure CSS with no per-frame JS - the material is picked once at build time, before the blocking computation starts, and persisted via `localStorage` so it actually advances between separate dialog openings rather than resetting every time). Each material has its own block gradient and matching chip colors. Verified directly: four consecutive simulated openings cycle ALUMINUM → WOOD → PLASTIC → ALUMINUM correctly, with each material's actual colors confirmed present on both the block and its chips.

**The material's name is now stamped into the center of the block**, and gets cut away right along with the material - this uses the exact same reveal mechanism already driving the material removal itself: the text sits underneath the reveal bands in paint order, so as each band grows to show the material being cut away, it naturally covers the same portion of the text too, with no separate logic needed.

**Another attempt at the reported border/seam**, since the previous GPU-layer-promotion fix didn't resolve it: switched from `transform: scale()` to CSS `zoom` for fitting the animation into the popup. This is a genuinely different scaling mechanism - `zoom` actually resizes the layout box rather than visually transforming a full-size box and then clipping it, which avoids the specific rendering path that was the leading suspect. `zoom` is non-standard but reliably supported in Chromium, which this embedded dialog runs on. Please verify visually whether this one actually resolves it - if not, the most likely remaining explanation is something more fundamental about how ncSender's webview handles this content, worth a closer look together rather than another blind guess from here.
...
- **The reveal is built from `scaleX(0)→scaleX(1)` on three height-banded overlays** (one per pass, each exactly a third of the material's height), not `clip-path` or `width` - `transform` is the one property with a fully guaranteed compositor-safe animation path, same reasoning as the rest of this animation's design.
- **Found and fixed a real sync bug during preview iteration**: the reveal was growing across the material's full width while the tool traveled a *different*, smaller distance in the same time - same start and end points, but visibly drifting out of sync in the middle of each pass, since a shorter distance covered in the same time moves slower. Fixed by making the tool's cutting-segment travel exactly match the band's own coordinate span (both now span precisely 110px to 1810px, the material's real left and right edges).
- **Each pass's depth is derived from the endmill's own tip geometry**, not an arbitrary Y value - the tool's vertical position for pass *N* is computed so its actual cutting tip lands exactly on that band's bottom edge, verified directly rather than eyeballed.
- Reconciled with everything already established: the tool still opens on left+up (v1.22.7), still travels clear of the material on both ends before diving in or lifting out (v1.22.6), and chips now get three separate visibility windows, one per pass, instead of one.
- The "removed" bands render as the modal's own actual surface color rather than literal `transparent` - since they're layered *inside* the material block, a truly transparent background would just show the block's own gradient through it and do nothing visually. Painting them with the modal's real background color is what makes a fully-revealed band read as an actual gap.

## v1.22.7 (EXPERIMENTAL — animation now opens on left+up instead of mid-cut)

**Re-anchored the animation cycle so it always opens on the tool sitting safely to the left and up, clear of the aluminum** - since the modal rebuilds this animation fresh every time it opens, whatever state is defined as the CSS animation's 0% keyframe is exactly what's shown the instant the popup appears. Previously that was left+down (already at cutting depth), which could read as if the tool was already mid-action. Now the visible sequence starting from open is: sitting left+up, plunge down, cut right across the material, retract up at the right, then rapid-return left to loop. Chip-visibility timing was re-verified against the new phase and needed no changes - it already lines up closely with the tool actually entering (114px vs the stock's 110px edge) and clearing (1813px vs 1810px) the material.
