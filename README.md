...
**Version**: 1.15.0 (EXPERIMENTAL layout — see note below)
**Category**: Utility
...
If a non-zero value is entered on an operation whose Notes don't say "internal" or "external" at all, that operation is left untouched rather than guessed at. A value of exactly 0.00 leaves that operation completely untouched, as if the Notes field weren't there. When at least one piece of an operation's geometry is successfully offset, one summary note like `( TWC Applied - Internal -0.10 )` is added under that operation's `( Notes: ... )` line.
